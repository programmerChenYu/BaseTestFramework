package io.github.programmerchenyu.listener;
...
public class ScreenshotListener implements IInvokedMethodListener {
    ...
    private void captureScreenshotAndAttach(ITestResult testResult) {
        try {
            ...
            String[] split = instance.getClass().getSuperclass().getName().split("\\.");
            String baseCaseClass = split[split.length-1];
            File screenshot;
            if ("AppBaseCase".equals(baseCaseClass)) {
                Field field = instance.getClass().getSuperclass().getDeclaredField("driver");
                field.setAccessible(true);
                AppiumDriver driver = (AppiumDriver) field.get(instance);
                screenshot = ((TakesScreenshot) driver).getScreenshotAs(OutputType.FILE);
            } else if ("WebBaseCase".equals(baseCaseClass)) {
                Field field = instance.getClass().getSuperclass().getDeclaredField("driver");
                field.setAccessible(true);
                WebDriver driver = (WebDriver) field.get(instance);
                screenshot = ((TakesScreenshot) driver).getScreenshotAs(OutputType.FILE);
            } else {
                throw new RuntimeException("The specified baseCaseClass is abnormal. Please select the baseCaseClass for AppBaseCase or WebBaseCase");
            }
